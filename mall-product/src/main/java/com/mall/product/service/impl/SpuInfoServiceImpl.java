...
public class SpuInfoServiceImpl extends ServiceImpl<SpuInfoDao, SpuInfoEntity> implements SpuInfoService {
    ...
    public void savesupInfo(SpuSaveVo vo) {
        ...
        if(skus!=null && skus.size()>0){
            skus.forEach(item->{
                ...
                //5、4）、sku的优惠、满减等信息：gmall_sms--->sms_sku_ladder、sms_sku_full_reduction、sms_member_price
    ...
    public void up(Long spuId) {
        ...
        //一、查出当前spuId对应的所有sku信息,品牌的名字
        List<SkuInfoEntity> skuInfoEntities = skuInfoService.getSkusBySpuId(spuId);

        //TODO 4、查出当前sku的所有可以被用来检索的规格属性   （因为规格属性都是按照spu来的，所以该商品下每个sku的属性一样  所以放在这里查一遍就可以了）
        ...
        List<Long> skuIdList = skuInfoEntities.stream()
                ...
                .collect(Collectors.toList());
        //TODO 1、发送远程调用，库存系统查询是否有库存   查库存就交给库存服务完成
        //若出现异常则默认让其有库存    为啥？？？p132最后
        ...
        //TODO 5、将数据发给es进行保存：mall-search    因为es的操作都在es服务中
